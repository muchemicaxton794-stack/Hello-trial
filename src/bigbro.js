...
async function runBigBroCommand({ command, args, jid, message: m, reply, isGroup, groupSettings, saveSettings, isAdmin, botIsAdmin, groupMeta, targetUsers, socket, botName: BOT_NAME, prefix: PREFIX, STARTED, formatRuntime, mention }) {
  ...
  switch (command) {
    case 'menu': {
      ...
      for (const button of ['ping', 'alive', 'time', 'music', 'sticker']) {
        await socket.sendMessage(jid, { text: `◉ ${PREFIX}${button}` });
      }
      return;
    }
    ...
    case 'sticker': {
      ...
      if (!imageMessage) return reply(m, 'Reply to an image to make a sticker.');
      try {
        ...
        console.error(error);
        return reply(m, 'Sticker failed. Please reply to a valid image.');
      }
      return;
    }
    ...
    case 'music': {
      ...
      const result = ytdl.validateURL(searchQuery) ? { videos: [{ url: searchQuery, title: 'YouTube audio' }] } : await yts(searchQuery);
      const video = result.videos?.[0];
      if (!video) return reply(m, 'No music result found.');
      const file = path.join(os.tmpdir(), `bigbrother-${Date.now()}.mp3`);
      try {
        ...
        await socket.sendMessage(jid, { audio: fs.readFileSync(file), mimetype: 'audio/mpeg', fileName: `${video.title.slice(0, 60)}.mp3` }, { quoted: m });
      } catch (error) {
        console.error(error);
        return reply(m, 'Music download failed. Try another song or URL.');
      } finally {
    ...
    case 'antikick': {
      ...
      groupSettings(jid)[command] = args[0].toLowerCase() === 'on';
      saveSettings();
      return reply(m, `✅ ${command} is now ${args[0].toLowerCase()}.`);
    }

    case 'add':
      return moderate('add', args[0] ? [jidFromNumber(args[0])] : []);

    case 'kick':
      return moderate('remove', targetUsers(m));

    case 'kickall': {
      if (!isGroup(jid)) return reply(m, 'This command only works in groups.');
      const users = (await groupMeta(jid)).participants.filter(p => !p.admin && p.id !== socket.user.id).map(p => p.id);
